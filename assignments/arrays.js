...
let oldCars = inventory.filter(function (carObj) {
  return carObj.car_year < 2000;
})
...
// let BMWAndAudi = [];
let BMWAndAudi = inventory.filter(function (carObj){
  if (carObj.car_make == 'Audi' || carObj.car_make == 'BMW' ){
    return carObj;
  }
});
// for (i=0; i<inventory.length; i++){
